...
export async function rafraichirLesCatalogues(formData: FormData): Promise<{
  ...
  /* Même garde que la tâche quotidienne : la lecture des collections
     allonge chaque marque, et un appel coupé à soixante secondes
     perdrait le bilan des trois. On rend la main à temps, et le
     navigateur reprend là où l'on s'est arrêté. */
  const debut = Date.now();
  let vues = 0;

  for (const marque of marques) {
    if (vues > 0 && Date.now() - debut > 35_000) break;
    vues++;
  ...
  return {
    ...
    resultats,
    parcourues: vues,
    restantes: Math.max(0, (count ?? 0) - (depuis + vues)),
  };
}
